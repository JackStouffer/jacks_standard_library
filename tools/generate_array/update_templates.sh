#!/bin/sh
#
# Regenerate the embedded template variables in generate_array.h
...
HEADER="$SCRIPT_DIR/generate_array.h"
...
# (from "static JSLImmutableMemory ..." through ");"), indented to match the header.
extract_var_block() {
    local var_name="$1"
    local txt_file="$2"
...
replace_var_block dynamic_header_template dynamic_array_header.txt
replace_var_block dynamic_source_template dynamic_array_source.txt
