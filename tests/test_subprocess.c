...
void test_jsl_subprocess_run_blocking_bad_parameters(void)
{
    ...
    int32_t exit_code = 0;
    JSLSubProcessRunResultEnum r = jsl_subprocess_run_blocking(NULL, &exit_code, NULL);
    TEST_INT32_EQUAL(r, JSL_SUBPROCESS_RUN_BAD_PARAMETERS);
...
void test_jsl_subprocess_run_blocking_spawn_failed(void)
{
    ...
    int32_t exit_code = 0;
    int32_t errno_out = 0;
    JSLSubProcessRunResultEnum r = jsl_subprocess_run_blocking(
        &proc,
        &exit_code,
        &errno_out
    );
    ...
    bool ok = (r == JSL_SUBPROCESS_RUN_SPAWN_FAILED)
        || (r == JSL_SUBPROCESS_RUN_SUCCESS && exit_code != 0);
    TEST_BOOL(ok);

    if (r == JSL_SUBPROCESS_RUN_SPAWN_FAILED)
...
void test_jsl_subprocess_run_blocking_exit_code(void)
{
    ...
    TEST_INT32_EQUAL(r, JSL_SUBPROCESS_RUN_SUCCESS);
...
void test_jsl_subprocess_run_blocking_already_started(void)
{
    ...
    int32_t exit_code = -1;
    JSLSubProcessRunResultEnum r = jsl_subprocess_run_blocking(&proc, &exit_code, NULL);
    TEST_INT32_EQUAL(r, JSL_SUBPROCESS_RUN_SUCCESS);

    r = jsl_subprocess_run_blocking(&proc, &exit_code, NULL);
    TEST_INT32_EQUAL(r, JSL_SUBPROCESS_RUN_ALREADY_STARTED);
